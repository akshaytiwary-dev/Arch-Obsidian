      ...
      <box
        ...
        $={(self) => {
          const updateVisibleChild = () => {
            const selectedWidget = globalSettings.peek().leftPanel.widget;
            const fallbackWidget = leftPanelWidgetSelectors[0];
            const selectedName = selectedWidget?.name || fallbackWidget.name;

            if (panelStack.get_child_by_name(selectedName)) {
              panelStack.set_visible_child_name(selectedName);
              return;
            }

            panelStack.set_visible_child_name(fallbackWidget.name);

            if (selectedName !== fallbackWidget.name) {
              setGlobalSetting("leftPanel.widget", fallbackWidget);
            }
          };
    ...
    >
      <togglebutton
        ...
        onToggled={(self) => {
          ...
          if (self.active) {
            setGlobalSetting("leftPanel.widget", leftPanelWidgetSelectors[0]);
            leftPanel.show();
            self.label = "";
          } else {
            leftPanel.hide();
