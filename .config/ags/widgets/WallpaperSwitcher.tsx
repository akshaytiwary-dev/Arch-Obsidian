...
const FetchWallpapers = async () => {
  try {
    execAsync([
      "bash",
      `${GLib.get_home_dir()}/.config/ags/scripts/get-wallpapers.sh`,
    ])
      .then((output) => {
...
function Display() {
      ...
      <box halign={Gtk.Align.CENTER}>
        <box class="all-wallpapers" spacing={5} hexpand>
          <For each={selectedWallpapers}>
            {(wallpaper) => {
              const handleLeftClick = async (self: Gtk.Button) => {
                ...
                try {
                  if (target === "sddm") {
                    await execAsync([
                      ...
                      "sed",
                      "-i",
                      `s|^background=.*|background=${wallpaper}|`,
                    ...
                    await execAsync([
                      "mkdir",
                      "-p",
                      `${GLib.get_home_dir()}/.config/wallpapers/lockscreen`,
                    ]);
                    await execAsync([
                      "cp",
                      wallpaper,
                      `${GLib.get_home_dir()}/.config/wallpapers/lockscreen/wallpaper`,
                    ]);
              ...
              const handleRightClick = async () => {
                setProgressStatus("loading");
                try {
                  await execAsync(["rm", "-f", toThumbnailPath(wallpaper)]);
                  await execAsync(["rm", "-f", wallpaper]);
                  notify({
                    summary: "Success",
  ...
  const resetButton = (
    <button
      ...
      onClicked={() => {
        setProgressStatus("loading");
        execAsync([`${GLib.get_home_dir()}/.config/hypr/wallpaper-daemon/reload.sh`])
        ...
        try {
          const filename = (
            await execAsync([
              ...
              "--file-filter=Media Files | *.png *.jpg *.jpeg *.webp *.gif *.mp4",
            ])
          ).trim();
          ...
          await execAsync([
            "mkdir",
            "-p",
            `${GLib.get_home_dir()}/.config/wallpapers/custom`,
          ]);
          await execAsync([
            "cp",
            filename,
    ...
    <window
      ...
      $={async (self) => {
        ...
        key.connect("key-pressed", (controller, keyval) => {
          if (keyval === Gdk.KEY_Escape || keyval === Gdk.KEY_w || keyval === Gdk.KEY_W) {
            self.hide();
            return true;
          }
          return false;
