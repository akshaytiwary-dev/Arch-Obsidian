...
type ButtonSetup = (button: Gtk.Button) => void;
...
const ResultsList = ({
  ...
  onLaunch: (app: LauncherApp) => void;
  getButtonSetup?: (index: number) => ButtonSetup;
}) => {
  return (
        ...
        <For each={results}>
          {(result, index) => (
            <AppButton
              ...
              buttonSetup={getButtonSetup?.(index.peek())}
              onLaunch={onLaunch}
            />
          )}
        </For>
...
export default ({
  ...
  let args: string[];
  const resultsButtons: Array<Gtk.Button | null> = [];
  const quickAppButtons: Array<Gtk.Button | null> = [];
  const historyButtons: Array<Gtk.Button | null> = [];
  ...
  const collectNavigableButtons = () =>
    [...resultsButtons, ...quickAppButtons, ...historyButtons].filter(
      (button): button is Gtk.Button =>
        Boolean(button) &&
        button.get_root() !== null &&
        button.get_visible() &&
        button.is_sensitive(),
    );

  const focusAdjacentButton = (current: Gtk.Button | null, delta: number) => {
    const buttons = collectNavigableButtons();
    if (buttons.length === 0) {
      return false;
    }

    if (!current) {
      const target = delta >= 0 ? buttons[0] : buttons[buttons.length - 1];
      target.grab_focus();
      return true;
    }
    ...
    const target = buttons[Math.min(nextIndex, buttons.length - 1)];
    target.grab_focus();
    return true;
  };

  const registerNavigableButton =
    (bucket: Array<Gtk.Button | null>, index: number): ButtonSetup =>
    (button) => {
      bucket[index] = button;
      ...
      keyController.connect("key-pressed", (_, keyval) => {
        if (keyval === Gdk.KEY_Up || keyval === Gdk.KEY_KP_Up) {
          return focusAdjacentButton(button, -1);
        }
    ...
    >
      ...
      <box class="app-launcher" spacing={10}>
        ...
        >
          ...
          >
            <Gtk.EventControllerKey
              onKeyPressed={(
                ...
                if (keyval === Gdk.KEY_Up || keyval === Gdk.KEY_KP_Up) {
                  return focusAdjacentButton(null, -1);
                }
          ...
          <scrolledwindow hexpand vexpand>
            <ResultsList
              results={Results}
              onLaunch={launchApp}
              getButtonSetup={(index) =>
                registerNavigableButton(resultsButtons, index)
              }
            />
          </scrolledwindow>
        </box>
        <box orientation={Gtk.Orientation.VERTICAL} spacing={10}>
          <QuickApps
            getButtonSetup={(index) =>
              registerNavigableButton(quickAppButtons, index)
            }
            onAfterLaunch={() => {
              if (parentWindowRef) {
                parentWindowRef.hide();
          ...
          <AppHistory
            ...
            onLaunch={launchApp}
            getButtonSetup={(index) =>
              registerNavigableButton(historyButtons, index)
            }
          />
        </box>
      </box>
