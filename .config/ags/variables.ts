...
const [barMenuOpen, _setBarMenuOpen] = createState<boolean>(false);
let barMenuHoldCount = 0;

export function holdBarOpen(active: boolean) {
  barMenuHoldCount = Math.max(0, barMenuHoldCount + (active ? 1 : -1));
  _setBarMenuOpen(barMenuHoldCount > 0);
}
...
export const musicData = createSubprocess(
  { text: " No playback", tooltip: "", class: "stopped", alt: "stopped" },
  ["python3", GLib.get_home_dir() + "/.config/waybar/scripts/waybar-music.py"],
  (out) => {
    try {
      return JSON.parse(out);
    } catch (e) {
      return { text: " No playback", tooltip: "", class: "stopped", alt: "stopped" };
    }
  }
);
