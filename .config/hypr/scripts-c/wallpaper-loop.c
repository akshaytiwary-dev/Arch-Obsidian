...
bool is_awww_running() {
    return system("pgrep -x awww-daemon >/dev/null 2>&1") == 0;
}
...
/*
 * Update wallpaper for a specific monitor and workspace ID
 */
void update_wallpaper_for_monitor(const char* monitor, int workspace_id) {
    ...
    if (!state) {
        ...
        notify_error("update_wallpaper_for_monitor", error_msg);
        return;
    }
 ...
 */
bool get_focused_monitor(char* focused_monitor, size_t size) {
    ...
    for (int i = 0; i < num_monitors; i++) {
        char search[128];
        snprintf(search, sizeof(search), "\"name\": \"%s\"", monitors_list[i]);
        char* pos = strstr(output, search);
        if (!pos) {
            snprintf(search, sizeof(search), "\"name\":\"%s\"", monitors_list[i]);
            pos = strstr(output, search);
        }
        
        if (pos) {
            char* focused_pos = strstr(pos, "\"focused\"");
            if (focused_pos) {
                char* colon = strchr(focused_pos, ':');
                if (colon) {
                    char* val = colon + 1;
                    while (*val == ' ' || *val == '\t') val++;
                    if (strncmp(val, "true", 4) == 0) {
                        if (strlen(monitors_list[i]) < size) {
                            strcpy(focused_monitor, monitors_list[i]);
                            return true;
                        }
                    }
                }
    ...
    if (strlen(monitors_list[0]) < size) {
        strcpy(focused_monitor, monitors_list[0]);
        return true;
    }
 ...
 * Iterates through all monitors and updates wallpapers
 */
void change_wallpaper() {
    ...
    for (int i = 0; i < num_monitors; i++) {
        ...
        if (workspace_id != -1) {
            update_wallpaper_for_monitor(monitor, workspace_id);
        }
    }
}
...
/*
 * Main daemon loop
 * 1. Waits for awww-daemon to start
...
int main() {
    ...
    printf("Waiting for awww-daemon to start...\n");
    while (!is_awww_running()) {
        sleep(1);
    }
    printf("awww-daemon detected, proceeding...\n");
    ...
    char focused_monitor[MAX_MONITOR_NAME] = "";
    get_focused_monitor(focused_monitor, sizeof(focused_monitor));
    
    char buffer[MAX_LINE_LEN];
    while (fgets(buffer, sizeof(buffer), sock_file)) {
        buffer[strcspn(buffer, "\n")] = 0;
        
        if (strncmp(buffer, "focusedmon>>", 12) == 0) {
            char ws_name[64] = "";
            char* comma = strchr(buffer + 12, ',');
            if (comma) {
                size_t mon_len = comma - (buffer + 12);
                if (mon_len < MAX_MONITOR_NAME) {
                    strncpy(focused_monitor, buffer + 12, mon_len);
                    focused_monitor[mon_len] = '\0';
                    
                    strncpy(ws_name, comma + 1, sizeof(ws_name) - 1);
                    ws_name[sizeof(ws_name) - 1] = '\0';
                    
                    int ws_id = atoi(ws_name);
                    if (ws_id > 0) {
                        update_wallpaper_for_monitor(focused_monitor, ws_id);
                    }
                }
            }
        } else if (strncmp(buffer, "workspace>>", 11) == 0) {
            int ws_id = atoi(buffer + 11);
            if (ws_id > 0 && focused_monitor[0] != '\0') {
                update_wallpaper_for_monitor(focused_monitor, ws_id);
            }
        }
    }
